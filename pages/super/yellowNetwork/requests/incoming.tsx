...
const IncomingRequests = dynamic(
  () =>
    import('@openware/opendax-web-sdk').then(
      (mod: any) => mod.IncomingRequests,
    ),
  {
    ssr: false,
  },
...
const Incoming: React.FC = () => {
  ...
  const breadcrumbItems = [
    ...
    { title: translate('page.tab.header.requests.incoming') },
  ]
  ...
  return (
    <>
      <Head>
        <title>
          {appTitle(translate('page.tab.header.requests.incoming'))}
        </title>
      </Head>
...
export default withAuth(withAdminAuth(Incoming))
